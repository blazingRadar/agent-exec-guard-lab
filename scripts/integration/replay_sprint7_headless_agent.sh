...
sg docker -c "docker ps -a --format '{{.Names}} {{.Status}}' | grep 'openhands-runtime-sprint7headless' || true" \
  >"$RUN_ROOT/runtime_container_status.txt" 2>"$RUN_ROOT/runtime_container_status.stderr"

container_name="$(awk '/openhands-runtime-sprint7headless/ {print $1; exit}' "$RUN_ROOT/runtime_container_status.txt")"
