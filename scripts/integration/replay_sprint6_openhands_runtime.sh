...
POLICY_IN_CONTAINER="/lab/policy/integration/openhands_runtime.allow.json"
GUARD_IN_CONTAINER="/lab/bin/usernotify_exec_guard"
WORKSPACE_IN_CONTAINER="/lab/proofs/sprint6_runs/$RUN_ID/workspace"

mkdir -p "$RUN_ROOT/workspace"
printf '%s\n' "$RUN_ROOT" >"$ROOT/proofs/sprint6_runs/latest_runtime_probe.txt"
printf 'sprint6-one-file-openhands-runtime\n' >"$RUN_ROOT/workspace/input.txt"
...
record_docker_metadata() {
  ...
  sg docker -c "docker create --name '$META_CONTAINER' -v '$ROOT:/lab:rw' -w /lab '$IMAGE' /bin/sh -c 'grep \"^Seccomp:\" /proc/self/status; grep \"^NoNewPrivs:\" /proc/self/status; command -v python3; command -v cat; command -v rm'" \
...
run_case "allowed_cat_workspace_file" 0 "$GUARD_IN_CONTAINER" --policy "$POLICY_IN_CONTAINER" /usr/bin/cat "$WORKSPACE_IN_CONTAINER/input.txt"
if grep -qx 'sprint6-one-file-openhands-runtime' "$RUN_ROOT/allowed_cat_workspace_file/stdout.txt"; then
  record "PASS" "allowed_cat_workspace_file_output" "workspace file read"
else
  record "FAIL" "allowed_cat_workspace_file_output" "workspace file output mismatch"
fi
if json_query "$RUN_ROOT/allowed_cat_workspace_file/stderr.txt" 'obj.get("decision") == "ALLOW" and obj.get("raw_exe") == "/usr/bin/cat"'; then
  record "PASS" "allowed_cat_workspace_file_decision" "ALLOW /usr/bin/cat recorded"
else
  record "FAIL" "allowed_cat_workspace_file_decision" "missing ALLOW /usr/bin/cat"
fi

run_case "guarded_child_proc_status" 0 "$GUARD_IN_CONTAINER" --policy "$POLICY_IN_CONTAINER" /usr/bin/cat /proc/self/status
if grep -q '^NoNewPrivs:[[:space:]]*1$' "$RUN_ROOT/guarded_child_proc_status/stdout.txt"; then
  record "PASS" "guarded_child_nonewprivs" "guarded child reports NoNewPrivs:1"
else
  record "FAIL" "guarded_child_nonewprivs" "missing guarded child NoNewPrivs:1"
fi
if grep -q '^Seccomp:[[:space:]]*2$' "$RUN_ROOT/guarded_child_proc_status/stdout.txt"; then
  record "PASS" "guarded_child_seccomp" "guarded child reports Seccomp:2"
else
  record "FAIL" "guarded_child_seccomp" "missing guarded child Seccomp:2"
fi
...
run_case "blocked_renamed_rm" 126 /bin/sh "/lab/proofs/sprint6_runs/$RUN_ID/blocked_renamed_rm.sh"
