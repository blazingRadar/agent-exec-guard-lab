...
Posture: read-only proof quality, audit-integrity, and CTO-readiness audit.
...
Medium: The F2 "closed for SIGTERM/INT/HUP" claim is a little too broad. The preserved proof shows the reproduced SIGTERM path works, but the handler uses `snprintf` inside a signal handler, which is not async-signal-safe: `guard/usernotify_exec_guard.c:95`. The report does caveat "denial-of-service, not survival," but line 12 still reads broader than the implementation can reliably promise: `proofs/SPRINT4_AUDIT_INTEGRITY_HARDENING_20260430.md:12`. CTO framing should say "reproduced SIGTERM/INT/HUP-style supervisor termination now emits a best-effort final record," unless the handler is made strictly async-signal-safe.
...
CTO-ready for a Sprint 4 evidence review, with the F2 wording tightened. Do not call it production-ready or Docker/OpenHands-ready, and the document itself does not overclaim that.
